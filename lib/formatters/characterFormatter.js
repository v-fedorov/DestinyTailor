...
CharacterFormatter.prototype.buildCharacter = function(character, data, callback) {
    // load the inventory
    var _this = this;
    this.bungie.getInventory(character.platform, character.accountId, character.id, function(err, result) {
        if (err) {
            return callback(err);
        };
...
CharacterFormatter.prototype.setItems = function(character, currentItem, remainingItems, callback) {
    if (!currentItem) {
        ...
        var _this = this;
        this.bungie.getInventoryItem(character.platform, character.accountId, character.id, currentItem.itemId, function(err, result) {
            if (err) {
                return callback(err);
            };
